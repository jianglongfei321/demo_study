 ...
 * 目标类切点函数        within()	                类名匹配串	      表示特定域下的所有连接点。如within(com.baobaotao.service.*)表示com.baobaotao.service包中的所有 连接点，也即包中所有类的所有方法，而within(com.baobaotao.service.*Service)表示在 com.baobaotao.service包中，所有以Service结尾的类的所有连接点。
 * 	               target()	                类名	              假如目标类按类型匹配于指定类，则目标类的所有连接点匹配这个切点。如通过target(com.baobaotao.Waiter)定义的切点，Waiter、以及Waiter实现类NaiveWaiter中所有连接点都匹配该切点。
 * 	               @within()	        类型注解类名	      假如目标类按类型匹配于某个类A，且类A标注了特定注解，则目标类的所有连接点匹配这个切点。
 ...
 * @AspectJ 除上表中所列的函数外，还有
 * call()、initialization()、 preinitialization()、
 * staticinitialization()、 get()、  set()、 handler()、
 * adviceexecution()、 withincode()、cflow()、cflowbelow()、
 * if()、 @this()以及@withincode()等函数这些函数在Spring中不能使用，
 * 否则会抛出IllegalArgumentException 异常。
 * @author jlf
 */
@Component
@Aspect
public class ExecutionAspect {
	 ...
	 * @param joinPoint
	 */
	@Before("execution(* com.roby.demo.spring.aop.TestService.echo(..))")
	 ...
	 * @param joinPoint
	 */
	@Before("@annotation(com.roby.demo.spring.aop.Spi)")
	public void beforeannotation(JoinPoint joinPoint) {
		System.out.println("before annotation aspect executed");
	}
	 ...
	 * @param joinPoint
	 */
	@Before("args(java.util.Map)")
	public void beforeArgs(JoinPoint joinPoint) {
	
		System.out.println(	joinPoint.getArgs()+"before args aspect executed");
	}
}
